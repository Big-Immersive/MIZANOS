...
class ReportDocumentService:
    ...
    def _add_title(doc: Document) -> None:
        ...
        if os.path.exists(logo_path):
            ...
            run.add_picture(logo_path, width=Inches(0.18))
        run = branding.add_run("  Generated by Mizan OS")
        run.font.size = Pt(8)
    ...
    def _add_portfolio_table(doc: Document, projects: list[dict], task_data: dict | None = None) -> None:
        ...
        for proj in projects:
            ...
            # Dashboard / Dev — use dashboard_url or first railway project link
            dash_url = proj.get("dashboard_url")
            ...
            if dash_url:
                ...
                cells[5].text = ""
                p = cells[5].paragraphs[0]
                run = p.add_run("[railway] ")
                run.font.size = Pt(7)
                run.font.color.rgb = RGBColor(100, 100, 100)
                _add_hyperlink(p, railway_link["url"], _shorten_url(railway_link["url"]))
            else:
                cells[5].text = "-"
