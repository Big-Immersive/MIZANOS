...
class ReportPDFService:
    ...
    def _add_title(pdf: _ReportPDF) -> None:
        # Branding top-right - single line
        pdf.set_font("Helvetica", "I", 7)
        pdf.set_text_color(130, 130, 130)
        pdf.cell(0, 4, "Generated by Mizan OS", align="R", new_x="LMARGIN", new_y="NEXT")
    ...
    def _add_portfolio_table(pdf: _ReportPDF, projects: list[dict], task_data: dict | None = None) -> None:
        ...
        for idx, proj in enumerate(projects):
            ...
            # Dashboard / Dev — use dashboard_url or first project link with railway tag
            dash_url = proj.get("dashboard_url")
            railway_link = next((pl for pl in project_links if "railway" in pl.get("name", "").lower()), None)
            if dash_url:
                pdf.set_text_color(5, 99, 193)
                pdf.cell(col_w[5], row_h, _shorten(dash_url)[:26], border=1, fill=fill, link=dash_url)
            elif railway_link:
                pdf.set_text_color(5, 99, 193)
                pdf.cell(col_w[5], row_h, _shorten(railway_link["url"])[:26], border=1, fill=fill, link=railway_link["url"])
            else:
                pdf.set_text_color(0, 0, 0)
